const PublisherService = require("../services/publisher.service"); // Hãy chắc chắn tên file service của bạn là publisher.service.js hoặc publishers.service.js
...
exports.findOne = async (req, res, next) => {
  try {
    ...
    if (!document) {
      return next(new ApiError(404, "Publisher record not found"));
    }
    return res.send(document);
  } catch (error) {
    return next(
...
exports.update = async (req, res, next) => {
  ...
  try {
    ...
    return res.send({ message: "Publisher record was updated successfully" });
  } catch (error) {
    return next(
      new ApiError(
        500,
...
exports.delete = async (req, res, next) => {
  try {
    ...
    return res.send({ message: "Publisher record was deleted successfully" });
  } catch (error) {
    return next(
      new ApiError(
        500,
